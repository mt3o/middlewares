import {MiddlewareStack} from "./types";
import {areTypesEquivalent} from "./index";
import {GenMiddlewareStack} from "./gen";

function isNever(value: any): boolean {
...
export function validateGenStack<In, Out>(stack: GenMiddlewareStack<In,Out>) {
    const errors: Error[] = [];
    for (let i = 0; i <= stack.length - 2; i++) {
        const self = stack[i];
        const next = stack[i + 1];
        ...
        if (!isNever(self.NextArg) && !areTypesEquivalent(next.MyArg, self.NextArg)) {
            errors.push(new Error(`Types don't match between self:${self.Name}.NextArg and next:${next.Name}.MyArg argument type`));
        }

        if (!areTypesEquivalent(self.NextResponse, next.MyResolved)) {
            errors.push(new Error(`Types don't match between self:${self.Name}.NextResponse and next:${next.Name}.MyResolved output type`));
        }
